...
def replace_return(content, func_name, new_return):
    # Find the start of the return statement in the function
    # We look for "return (" after the function declaration
    func_idx = content.find(func_name)
    if func_idx == -1:
        print(f"Function {func_name} not found")
        return content
    
    return_idx = content.find('return (', func_idx)
    if return_idx == -1:
        # Check for createPortal return
        return_idx = content.find('return createPortal(', func_idx)
    
    if return_idx == -1:
        print(f"Return not found for {func_name}")
        return content
...
# Pix payment has two returns, we'll use regex for the whole block instead of just one return
...
content = replace_return(content, 'const ApplePayButton', apple_return)
...
content = replace_return(content, 'export const CheckoutModal', modal_return)

# Also fix the inputStyle definition inside AppmaxCCPayment so it doesn't cause TS errors
...
print("CheckoutModal.tsx rewritten successfully!")
