...
/* ─── Translations ───────────────────────────────────── */
const NEW_EU_DESC_PT = 'Tenha acesso ao aplicativo de importação da Connect Academy e importe direto da China, sem quantidade mínima e com todo o processo simplificado. São +30 milhões de produtos, +1.500 fabricantes e grandes marcas como Apple, DJI, Samsung, Nike, Adidas e muitas outras, além de rastreamento em tempo real, Minerador com IA e ferramentas exclusivas. Tudo o que você precisa para importar, em um só aplicativo.';
const NEW_EU_DESC_ES = 'Accede a la aplicación de importación de Connect Academy e importa directo desde China, sin cantidad mínima y con todo el proceso simplificado. Son +30 millones de productos, +1.500 fabricantes y grandes marcas como Apple, DJI, Samsung, Nike, Adidas y muchas más, además de rastreo en tiempo real, Minerador con IA y herramientas exclusivas. Todo lo que necesitas para importar, en una sola app.';
const NEW_EU_DESC_FR = "Accédez à l'application d'importation de Connect Academy et importez directement depuis la Chine, sans quantité minimale et avec tout le processus simplifié. Ce sont +30 millions de produits, +1 500 fabricants et de grandes marques comme Apple, DJI, Samsung, Nike, Adidas et bien d'autres, en plus du suivi en temps réel, du Minerador avec IA et des outils exclusifs. Tout ce dont vous avez besoin pour importer, dans une seule application.";
const NEW_EU_DESC_NL = 'Krijg toegang tot de importapplicatie van Connect Academy en importeer direct uit China, zonder minimumhoeveelheid en met een volledig vereenvoudigd proces. Het zijn +30 miljoen producten, +1.500 fabrikanten en grote merken zoals Apple, DJI, Samsung, Nike, Adidas en nog veel meer, plus realtime tracking, Minerador met AI en exclusieve tools. Alles wat u nodig heeft om te importeren, in één app.';
const NEW_EU_DESC_DE = 'Erhalten Sie Zugang zur Importanwendung von Connect Academy und importieren Sie direkt aus China, ohne Mindestmenge und mit einem vollständig vereinfachten Prozess. Das sind +30 Millionen Produkte, +1.500 Hersteller und große Marken wie Apple, DJI, Samsung, Nike, Adidas und viele mehr, dazu Echtzeit-Tracking, KI-gestützter Minerador und exklusive Tools. Alles, was Sie für den Import brauchen, in einer einzigen App.';
const NEW_EU_DESC_IT = "Accedi all'applicazione di importazione di Connect Academy e importa direttamente dalla Cina, senza quantità minima e con tutto il processo semplificato. Sono +30 milioni di prodotti, +1.500 produttori e grandi marchi come Apple, DJI, Samsung, Nike, Adidas e molti altri, oltre a tracciamento in tempo reale, Minerador con IA e strumenti esclusivi. Tutto ciò di cui hai bisogno per importare, in un'unica app.";

const NEW_BR_DESC = 'Tenha acesso ao aplicativo de importação da Connect Academy e importe direto da China, sem quantidade mínima e com todo o processo simplificado. São +30 milhões de produtos, +1.500 fabricantes e grandes marcas como Apple, DJI, Samsung, Nike, Adidas e muitas outras, além de rastreamento em tempo real, Minerador com IA e ferramentas exclusivas. Tudo o que você precisa para importar, em um só aplicativo.';

const TR: Record<string, Record<string, string>> = {
  'pt-BR': {
    hero_title: 'Aprenda importar da China sem burocracias alfandegárias.',
    hero_sub: NEW_BR_DESC,
...
export const NovaPage: React.FC<{ showExperience?: boolean }> = ({ showExperience = false }) => {
  ...
  // Autoplay de videos — iOS Safari só libera .play() com gesture do usuário
  useEffect(() => {
    ...
    // Tenta rodar imediatamente (funciona para videos visíveis no load)
    const initTimer = setTimeout(() => {
      allVideos().forEach(v => playVideo(v));
    }, 150);

    // Configura o observer de scroll
    const setupObserver = () => {
      ...
      allVideos().forEach(v => scrollObserver!.observe(v));
    };

    // No primeiro toque/scroll o iOS libera .play() para todos
    ...
    document.addEventListener('scroll', onFirstGesture, { passive: true });
              ...
              <>
                ...
                <form
                  onSubmit={e => {
                    ...
                    setCatalogEmail('');
                    // 3. Salva lead em background — IIFE async garante execução no Supabase v2
