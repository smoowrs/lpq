...
# Let's fix the ApplePayButton issue by fetching it from the git repository since we haven't committed yet
import subprocess
try:
    original = subprocess.check_output(['git', 'show', 'HEAD:src/components/CheckoutModal.tsx']).decode('utf-8')
except Exception:
    # if it fails, we will just manually restore ApplePayButton block
    pass

# We will just rewrite the file from scratch using the original file in git to be safe, since there might be other issues.
original = subprocess.check_output(['git', 'show', 'HEAD:src/components/CheckoutModal.tsx']).decode('utf-8')
...
def replace_return(content, func_name, new_return, search_string="return ("):
    ...
    # Start looking for the specific return string
...
# Read definitions from previous script using import or just embed them
...
# Need a more specific search for ApplePayButton to avoid hitting the useEffect return
content = replace_return(content, 'const ApplePayButton', apple_return, search_string="return (")
# Let's fix ApplePayButton by matching exactly what we know it returns
