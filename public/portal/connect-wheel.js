...
`;
  function init(options = {}) {
    ...
    dialog.innerHTML = `<div class="layout"><div class="celebration" aria-hidden="true"></div><button class="sound" type="button" aria-label="Desativar som" aria-pressed="true">♪</button><button class="close" type="button" aria-label="Fechar roleta" autofocus>×</button><div class="content"><div class="brand"><img src="/portal/assets/logo.png" class="brand-logo" alt="Connect Academy" /></div><p class="kicker">UMA SURPRESA DA CONNECT</p><h2 class="heading" id="cw-title">Gire. Revele. <em>Aproveite.</em></h2><div class="offer"><strong class="amount"></strong><span class="off">OFF.</span></div><p class="intro" id="cw-intro">Explore mais de 30 milhões de produtos, encontre 1.500 fornecedores verificados e conecte-se ao maior grupo de networking do Brasil.</p><button type="button" class="primary spin"><span>Girar e desbloquear desconto</span><span class="arrow" aria-hidden="true">↗</span></button><div class="result-panel" hidden><div class="coupon-row"><span class="code"></span><button class="copy" type="button">Copiar</button></div><a class="primary checkout"><span>Usar meu desconto</span><span class="arrow" aria-hidden="true">↗</span></a></div><p class="micro">30% garantidos. Confira as condições.</p><p class="demo" hidden>PRÉVIA · Cupons ilustrativos, sem validade.</p><details class="conditions"><summary>Como funciona e condições</summary><p>Esta roleta revela uma oferta de até 30% OFF.</p><ul class="chance-list"></ul><p class="terms"></p></details><p class="status" role="status" aria-live="polite"></p></div><div class="visual"><span class="spark one" aria-hidden="true">✳</span><span class="spark two" aria-hidden="true">✧</span><span class="badge">30% GARANTIDOS</span><div class="wheel-wrap"><div class="rim"></div><div class="disk" aria-hidden="true"></div><div class="hub" aria-hidden="true">✳</div><svg class="pin" viewBox="0 0 32 44" aria-hidden="true"><path d="M3 4Q16 -3 29 4L25 23L16 42L7 23Z" fill="#254bff" stroke="white" stroke-width="3"/><circle cx="16" cy="11" r="3" fill="white"/></svg></div><div class="orbit-label"><b aria-hidden="true">✦</b><span>Um presente para o seu próximo passo.</span></div></div></div>`;
    root.append(dialog); document.body.append(host);
    const $ = s => root.querySelector(s);
    function updateSound(){ $('.sound').textContent=soundOn?'♪':'♪̸'; $('.sound').setAttribute('aria-label',soundOn?'Desativar som':'Ativar som'); $('.sound').setAttribute('aria-pressed',String(soundOn)); }
    ...
    function tickFrame(now){
      ...
      if(matrix!=='none'){const values=matrix.slice(7,-1).split(',').map(Number);const angle=(Math.atan2(values[1],values[0])*180/Math.PI+360)%360;const sector=Math.floor(angle/60);if(sector!==lastSector&&now-lastTick>38){lastSector=sector;lastTick=now;tone(1450,0,.038,.04,'triangle');}}
    ...
    function confetti() {
      ...
      for (let i = 0; i < 55; i++) {
        ...
        p.style.setProperty('--x', (Math.random() * 680 - 440) + 'px'); p.style.setProperty('--y', (Math.random() * 350 + 70) + 'px'); p.style.setProperty('--r', (Math.random() * 900 - 450) + 'deg'); p.style.setProperty('--delay', (Math.random() * .15) + 's'); p.style.setProperty('--color', ['#254bff','#9bb1ff','#6b87ff','#becaff','#d9b96d'][i % 5]); $('.celebration').append(p);
      }
