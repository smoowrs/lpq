...
@keyframes cw-enter{from{transform:translateY(100vh)}to{transform:translateY(0)}}@keyframes cw-tick{0%,100%{transform:translateX(-50%) rotate(0)}50%{transform:translateX(-50%) rotate(-14deg)}}@keyframes cw-pop{0%{transform:scale(.96)}50%{transform:scale(1.025)}100%{transform:scale(1)}}@keyframes cw-confetti{0%{opacity:1;transform:translate(0,0) rotate(0)}100%{opacity:0;transform:translate(var(--x),var(--y)) rotate(var(--r))}}
...
`;
  function init(options = {}) {
    ...
    function tickFrame(now){
      ...
      if(matrix!=='none'){const values=matrix.slice(7,-1).split(',').map(Number);const angle=(Math.atan2(values[1],values[0])*180/Math.PI+360)%360;const sector=Math.floor(angle/60);if(sector!==lastSector&&now-lastTick>38){lastSector=sector;lastTick=now;tone(1450,0,.038,.04,'triangle'); if(navigator.vibrate) navigator.vibrate(10);}}
